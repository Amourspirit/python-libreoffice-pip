...
class Equals:
    ...
    def get_is_match(self, vstr: str) -> bool:
        ...
        if vlen < 3 or vlen > 10:
            return False
        return self._starts_with_equal(vstr)
    ...
    def get_version_is_valid(self, check_version: str, vstr: str) -> int:
        ...
        Returns:
            int: ``0`` if the check_version is equal to vstr, ``1`` if the check_version is greater than vstr.
                ``-1`` if the check_version is less than vstr. ``-2`` if the version is invalid.
        """
        try:
