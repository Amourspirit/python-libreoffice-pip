        ...
        try:
            ...
            if cmd == "UninstallItems":
                ...
                if not self.dialog_handler.uninstall_pkgs:
                    ...
                    title = self.dialog_handler._resource_resolver.resolve_string("mbtitle001")
                    msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg001")
                    _ = MessageDialog(
                    ...
                    return
                title = self.dialog_handler._resource_resolver.resolve_string("mbtitle002")
                msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg002")
                ...
                if result == 2:
                    ...
                    #     title = self.dialog_handler._resource_resolver.resolve_string("mbtitle001")
                    #     msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg003")
                    #     _ = MessageDialog(
                    #         self.dialog_handler.ctx,
                    #         title=title,
                    #         message=msg,
                    ...
                    #     msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg004")
                    #     _ = MessageDialog(
                    #         self.dialog_handler.ctx,
                    #         title=title,
                    #         message=msg,
...
class ButtonOthListener(XActionListener, unohelper.Base):
    ...
    def _action_info(self, ev: Any) -> None:  # noqa: ANN401
        ...
        title = self.dialog_handler._resource_resolver.resolve_string("mbtitle001")  # Info
        msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg005")
        if self.dialog_handler.config.cmd_clean_file_enabled:
            if self.dialog_handler.is_win:
    ...
    def _action_copy_cmd_ps(self, ev: Any) -> None:  # noqa: ANN401
        ...
        title = self.dialog_handler._resource_resolver.resolve_string("mbtitle006")
        msg = self.dialog_handler._resource_resolver.resolve_string("mbmsg006")
...
class OptionsDialogUninstallHandler(DialogBase, XContainerWindowEventHandler, unohelper.Base):
        ...
        try:
            ...
            if ev_name == "initialize":
                btn_info = self.create_button(
                    "Info",
                    self.resource_resolver.resolve_string("mbtitle001"),  # Info
                    x=138,
                    y=self._current_y,
                ...
                btn_model.HelpText = self.resource_resolver.resolve_string("mbtitle001")
                ...
                if self._config.cmd_clean_file_enabled:
                    self._add_copy_cmd()
    ...
    def _add_copy_cmd(self) -> None:
        ...
        btn = self.create_button(
            "CopyCmd",
            self.resource_resolver.resolve_string("ctl001"),
            x=17,
            y=self._current_y,
            width=self._button_width,
            height=self._button_height,
        ...
        btn_model.HelpText = self.resource_resolver.resolve_string("ctlTip001")
